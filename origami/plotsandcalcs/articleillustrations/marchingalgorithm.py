...
def plot_marching_algorithm():
    ...
    draw_lengths(ax, quads, (1, 1), (1, 2), '$ u_{00} $', 0.85, 0.0, -0.15, -0.1, -0.05)
    draw_lengths(ax, quads, (1, 1), (2, 1), '$ v_{00} $', 0.8, 0.11, -0.03, -0.03, -0.01)
    draw_lengths(ax, quads, (1, 4), (1, 5), '$ u_{03} $', 1.0, 0.05, -0.1, 0.0, -0.05)
    draw_lengths(ax, quads, (2, 1), (3, 1), '$ v_{10} $', 0.75, 0.19, -0.03, 0.0, -0.05)
...
def draw_lengths(ax: Axes, quads: QuadrangleArray, ind0, ind1, text,
    ...
    arrow = FancyArrowPatch(start, end, linestyle='-', color='0.35', arrowstyle='|-|', mutation_scale=10, lw=1.5,
                            zorder=5)
    ax.add_patch(arrow)
    middle = (start + end) / 2
